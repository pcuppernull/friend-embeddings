...
```{r}
# make_projection(paths[3], dims, dim_names, country_list)
```

### Dim Direction Test
```{r}
results <- map_dfr(paths[21:25], ~make_projection(.x, dims, dim_names, direction_test))

write_csv(results, 'documents/projections/dim_direction_test_V3.csv')
```
...
paths
      ...
      #Clean Model
      model <- model %>%
            separate(V1, into = as.character(0:100), sep = " ")
      #Word List
      words <- model %>%
            select("0") %>%
            rename(word = "0")
